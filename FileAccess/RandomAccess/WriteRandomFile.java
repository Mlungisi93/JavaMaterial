...
/*
RandomAccessFile method seek positions file-position pointer
 to a specific location in a file relative to beginning of file
Size of each record is known, so location in file of a specific record can be found
 by multiplying size of record with number of record
Once location known, new record data can be written
 without worrying about rest of file, as each record is always same size

 */
                    output.seek((accountNumber - 1) * // position to proper
                            RandomAccessAccountRecord.SIZE); // location for file 72
                    record.write(output);
                } // end if
                else
